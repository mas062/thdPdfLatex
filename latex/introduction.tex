...
\begin{table}[tbp]
  ...
  \begin{tabular}{|p{.35\linewidth}|p{.15\linewidth}|p{.15\linewidth}|}
	...
	Cell x dimension& $37.5~m$&$75~m$\\% \hline
	Cell y dimension& $37.5~m$&$75~m$\\% \hline	
	Cell z dimension& $1~m$&$4~m$\\% \hline
    \hline
...
\includegraphics[width=0.3 \linewidth,natwidth=524bp,natheight=397bp]{./figurer/facies_3.pdf}\\(a) Coastal Plain&(b) Upper Shoreface&(c) Lower Shoreface\\
...
(a) Coastal Plain&(b) Upper Shoreface\\
...
(c) Lower Shoreface& (d) Offshore Transition\\
...
The resistance of a porous medium against flow results in a flux that can be
...
Substituting $v$ from Equation \ref{eq:darcy_c} into Equation
\ref{eq:main_o} gives:
\begin{equation}
  \frac{d}{dt}\int_{\Omega}\phi\rho d\tau-\int_{\Gamma}\rho
\left(\frac{K}{\mu}\cdot (\nabla{P}+\rho g\nabla{Z})\right) \cdot n d\sigma=\int_{\Omega}qd\tau.
  \label{eq:main}
...
We assume that $(\phi \rho)$ satisfies the Leibniz integration rule conditions. Then, the derivative in the first term of Equation \ref{eq:main_o} can enter the integral. The second term in Equation \ref{eq:main_o} can be converted into an integration
...
Equation \ref{eq:darcy_c} into Equation \ref{eq:diff}, assuming the spatial density variation to be zero (i.e., $\nabla \rho=0$), and by
...
the pore can move them out of the pore. This level of $\mbox{CO}_2$ saturation  is
...
saturation. The plot shows a typical Van Genuchten curve for capillary pressure. The scale in the vertical axis is only for demonstration. For application, see for example \cite{luckner1989consistent} and \cite{ippisch2006validity}.}
  \label{fig:pc}
\end{figure}
...
saturation ($ S_{\alpha}$). Also the flux is the phase flux
$v_{\alpha}$, and the source/sink term must be written for the phase mass rate
$q_{\alpha}$.

For phase $\alpha=\{w~\mbox{for water}, \mbox{CO}_2\},$ we have:
\begin{equation}
  \frac{d}{dt}\int_{\Omega}\phi\rho_{\alpha}
S_{\alpha}d\tau+\int_{\Gamma}\rho_{\alpha} v_{\alpha} \cdot
n d\sigma=\int_{\Omega}qd\tau.
  \label{eq:2phs}
\end{equation} Darcy equation for two phases $\alpha=\{w~\mbox{for water}, \mbox{CO}_2\}$
...
the phase pressure, $\rho_{\alpha}$ is the phase density, $g$ is the gravitational acceleration, and $Z$ is the elevation.



Similar to Equation \ref{eq:diff}, differential form of mass balance equation
for each of phases $\alpha=\{w~\mbox{for water},\mbox{CO}_2\}$ is as follows:
\begin{equation}
...
strong coupling of saturation and phase flux. This nonlinearity can lead to
...
\begin{equation}
 f_w(S_w)=\frac{\frac{K_{rw}}{\mu_w}}{\frac{K_{rw}}{\mu_{w}}+\frac{K_{r{co_2}}}{
\mu_{co_2}}}.
 \label{eq:fw}
\end{equation} The total flux is defined as:
\begin{equation}
 v_t=v_w+v_{co_2}.
 \label{eq:VT}
...
Herein, we give the method reported in \cite{fayers1959effect}. If we assume incompressible flow (i.e., constant phase density) and medium (i.e., constant porosity) in a one-dimensional domain $\Omega$
...
flow becomes:
 \begin{equation}
  v_w=-K\frac{k_{rw}}{\mu_w}(\frac{\partial P_w}{\partial x} + \rho_w g z).
  \label{eq:D1d}
 \end{equation} Here, $x$ is the spatial direction in domain $\Omega$, $z$ is the vertical elevation and $g$ is the gravitational
...
. We can define the dimensionless variables as follows:
 \begin{equation}
  ...
  \label{eq:var*}
 \end{equation} where $L$ is a length constant in the problem, $v_t$ is the total flux, and $\pi_c$ is a
capillary pressure normalizing constant. For incompressible fluids and a constant total flow rate, the total flux $v_t$ equals the summation of phase fluxes:
\begin{equation}
v_t=v_{w}+v_{nw}.
\label{eq:totalFlux}
\end{equation}
 After reformulating Equation
...
have:
 ...
 \end{equation}
 \begin{equation}
  C(S_w)=N_{C}F_wk_{rnw}\frac{\partial P_{c}}{\partial S_w},
  \label{eq:PcCont}
 \end{equation} wherein: 
 \begin{equation}
  F_w=\frac{\lambda_{w}}{\lambda_{w}+\lambda_{nw}},
  \label{eq:Fn}
  \end{equation}
\begin{equation}
 ...
 and 
 \begin{equation}
  N_G=\frac{K(\rho_w-\rho_{nw})gz}{\mu_{nw}v_t}.
  \label{eq:Fn}
 \end{equation} 
 Here,
  \begin{equation}
   \lambda_{w}=\frac{k_{rw}}{\mu_{w}},
   \label{eq:MobW}
 \end{equation}
 and 
 \begin{equation}
   \lambda_{nw}=\frac{k_{rnw}}{\mu_{nw}}.
   \label{eq:MobnW}
 \end{equation}
 Having these definitions, Equation \ref{eq:2p1d} reshapes into:
 \begin{equation}
  \frac{\partial S_w}{\partial T^*}+\frac{d G(S_w)}{d S_w}\frac{\partial S_w}{\partial X^*}+\frac{\partial}{\partial X^*}\left( C(S_w)\frac{\partial
...
Some important conclusions in the literature \cite{fayers1959effect,dong1999effect,rosado2007analysis,bentsen1993effect,yortsos1983analytical} from sensitivity studies on
...
$\mbox{CO}_2$ phases, $H$ is the formation thickness, and $D_c$ is the molecular diffusion coefficient of $\mbox{CO}_2$
...
component $\mbox{c}$ in phase $\alpha$ and $v_{\alpha}$ is the flux of phase $\alpha$ \cite{elenius2010co2}. 
...
regime.]{\label{fig:injF}\includegraphics[width= 0.8
\linewidth]{./figurer/forceBalance_injection}}\\
  \subfloat
  [Long-term migration flow regime.]{\label{fig:migF}\includegraphics[width= 0.8
...
\begin{equation}
 T_{ab}=(\frac{1}{T_a}+\frac{1}{T_b})^{-1}.
 \label{eq:Thav}
\end{equation} Each half transmissibility, for example $T_{a}$, is calculated by an inner product
...
of the fluid, i.e., to changes in volume at the surface and at the geological formation condition. It is defined in this way in the simulator to consider cases where a
...
\begin{figure}
 ...
 \caption{In the models used for flow simulation, the top, bottom, and upper side boundaries are closed and the rest are open to the flow. Arrows point to the boundaries and their color indicates if the boundary is open (green) or closed (red). Colors on the grid show the depth of different locations. The injector location is depicted by a black line.}
 \label{fig:BDRY}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=0.56 \linewidth]{./figurer/bkw} 
  %
...
\begin{figure}
  \centering
  \includegraphics[width=0.6 \linewidth]{./figurer/LR_2} 
  %
...
\clearpage
Results are discussed by comparing all cases in plots. However, the conclusions are made based on detailed flow study in some picked cases. For example, Figures \ref{fig:Kz} to \ref{fig:FOSEOI} show the rock properties and \coo\ distribution in the domain at end of injection (i.e., after $30$ years from the simulation start) and end of simulation (i.e., after $100$ years from the simulation start) in two different cases. The heterogeneity description of the selected cases, called A and B, is given in Table \ref{tab:AaB}.

\coo\ distribution in Figures \ref{fig:COEOI} and \ref{fig:COEOS} show that heterogeneity in case B has enhanced the lateral flow compared to case A. Direction of the flow can be seen in Figure \ref{fig:FOSEOI}. It is clear that heterogeneity can influence the imbibition and drainage process during and after injection. This, in turn, impacts the residual trapping process. The \coo\ plume can follow various paths through heterogeneities. In Figure \ref{fig:FOSEOI}a, the up-dip migration of \coo\ toward the crest pushes some volumes to move down-dip through high transmissibility channels.
...
\begin{figure}
\begin{tabular}{cc}
\includegraphics[width=0.4\textwidth]{./figurer/C02222_LogKz_pers}&
\includegraphics[width=0.4\textwidth]{./figurer/C03211_LogKz_pers} \\ 
(a) Perspective view of model A&(b) Perspective view of model B\\
\includegraphics[width=0.4\textwidth]{./figurer/C02222_LogKz_slcx}&
\includegraphics[width=0.4\textwidth]{./figurer/C03211_LogKz_slcx}
\\(c) The X slice at the injection point (see (a)).&
(d) The X slice at the injection point (see (b)).\\
\includegraphics[width=0.4\textwidth]{./figurer/C02222_LogKz_slcy}&
\includegraphics[width=0.4\textwidth]{./figurer/C03211_LogKz_slcy}
...
(f) The Y slice at the injection point (see (b)).
\end{tabular}
\caption{Flow sign in the X direction at the end of injection for two selected cases. The left plots correspond to case A in Table \ref{tab:AaB}, and the right plots belong to case B. Red color corresponds to down-dip direction, blue to up-dip direction, and green represents the stagnant fluid.}
\label{fig:FOSEOI}
\end{figure}
%\clearpage
...
\end{tabular}
\caption{Mobile \coo\ distribution at different times for two selected cases. Cases A and B are described in Table \ref{tab:AaB}. Compare with Figures \ref{fig:Kz} and \ref{fig:Ky} for transmissibility values in different directions. Colors represent the same ranges shown in the colorbar.}
\label{fig:COplume}
\end{figure}
...
polynomials. In particular, we use arbitrary polynomial chaos (aPC) expansions, which consists of orthogonal polynomial basis that are constructed  according
...
space of approximating polynomial basis. This expression is specified by constant coefficients $c_i$:
%
\begin{equation}
...
\end{equation}
%
For simplicity, we proceed with describing the procedure for one dimensional orthogonal basis. The high-dimensional basis can be obtained using tensor products on one-dimensional basis. Therefore, we consider the polynomial $P^{(k)}$ of degree $k$ in the random variable $\theta$:
\begin{equation}
P^{(k)}(\theta)=\underset{j=0}{\overset{k}{\sum}}p_{j}^{(k)}\theta^{j}.
\end{equation}
where $k$ can vary between $0$ and $d$. Polynomials $P^{(k)}$ are orthogonal, if every pair of them fulfill the following condition:

\begin{equation}
\int_{I\in\Omega}P^{(l)}P^{(m)}d\tau(\theta)=\delta_{lm},\label{eq:orth}\end{equation} where $\delta$ is the Kronecker delta function and $\tau$ is
the measure for input variable space. If we modify the polynomials $P^{(k)}$ such that the coefficient of leading terms with the highest degree becomes one (i.e., $p_k^{(k)}=1$), the orthogonal polynomial basis satisfying Equation~(\ref{eq:orth}) can be obtained from the solution of the following linear system of equations \cite{oladyshkin2011concept}: 
%
\begin{equation}
...
%
\begin{equation}
\mu_{k}=\int_{\theta\in\Omega}\theta^{k}d\tau(\theta).\label{eq:mnt}
\end{equation}
%
Thus, arbitrary polynomial chaos expansion based on Equation~\ref{eq:orthsys}
only demands the existence of a finite number of moments, and does not require the exact knowledge or even existence of probability density functions. If the moments of $\theta$ are evaluated directly from a data set of limited size or from a discrete probability distribution featuring a finite number of possible outcomes, there need to be
$k$ or more distinct values in the data set or distribution. An interesting aspect is that only moments up to twice the order of the expansion are important. This means that there is no need for any kind of assumption for data probability distribution leading to subjectivity artifacts as discussed earlier. 
...
components (for instance, polynomial basis), it is easier to decompose the
...
decompose the solution and for $n$-dimensional input with Gaussian distribution $\Phi_n$ in
...
\begin{equation}
D=\mbox{V}[F(\Theta)]=\int_{R^{n}} F^2(\Theta) d\Theta-F_0^2,
\label{eq:SobD}\end{equation} that can be decomposed into: 
\begin{equation}
D_i=\int_{R^{1}}{F_i}^2(\theta_i) d\theta_i,
\label{eq:SobDi}\end{equation} and
\begin{equation} 
D_{i,j}=\int_{R^{2}}{F_{ij}}^2(\theta_{i},\theta_{j}) d\theta_{i}d\theta_{j}.
...
$\theta_2$ is the most influential parameter, because it appears in three terms,
