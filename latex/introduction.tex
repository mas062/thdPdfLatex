...
The second term in Equation \ref{eq:main_o} can be converted into an integration
...
\begin{equation}
 \frac{d}{dt}(\phi\rho_{\alpha}S_{\alpha})+\nabla \cdot
...
the wetting phase:
 ...
 \begin{equation}
  \phi \frac{ds_{w}}{dt}+ \frac {dv_w} {dx}=0,
  \label{eq:2p1d}
 \end{equation} and Darcy equation for one
dimension
flow becomes:
 \begin{equation}
  v_w=-K\frac{k_{rw}}{\mu_w}(\frac{dP_w}{dx} + \rho_w g z).
  \label{eq:D1d}
...
have:
 ...
 \end{equation}
 \begin{equation}
  C(S_w)=N_{C}F_wk_{rnw}\frac{dP_{c}}{dS_w},
  \label{eq:PcCont}
...
\begin{equation}
 ...
 \begin{equation}
  \frac{dS_w}{d T^*}+\frac{dG(S_w)}{dS_w}\frac{d
S_w}{d X^*}+\frac{d}{d X^*}\left( C(S_w)\frac{d
S_w}{d X^*}\right)=0.
  \label{eq:2p1dN}
...
\begin{equation}
\frac{d}{d t} \underset{\alpha}{\sum}\phi S_\alpha \rho_\alpha
...
Polynomials $\Pi_i$ in the Expansion \ref{eq:exp} are orthogonal, i.e., every pair of them fulfill the following condition:

\begin{equation}
\int_{I\in\Omega}\Pi_{l}\Pi_{m}d\tau(\Theta)=\delta_{lm},\label{eq:orth}\end{equation} where $\delta$ is the Kronecker delta function and $\tau$ is
the measure for input variable space. Constructing the orthogonal basis can result in expressions based on statistical moments of data. For simplicity, we proceed with describing the procedure for one dimensional orthogonal basis, since the high-dimensional basis can be obtained using tensor products on one-dimensional basis.  

 Provided that the coefficient of leading terms with the highest degree is one, the orthogonal polynomial basis satisfying Equation~(\ref{eq:orth}) can be obtained from the solution of the following linear system of equations \cite{oladyshkin2011concept}: 
%
\begin{equation}
...
\end{equation}
%
With basis without coefficient one in the leading term, it is possible to work with orthonormal basis calculated from them. Thus, arbitrary polynomial chaos expansion base on Equation~\ref{eq:orthsys}
only demands the existence of a finite number of moments, and does not require the exact knowledge or even existence of probability density functions. An
